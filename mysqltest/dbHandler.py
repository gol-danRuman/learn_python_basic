...
class InstructorMadeAssignment(Base):
    ...
    def __repr__(self):
        # return "<Assignment(id='%s',name='%s', courseid='%s')>" % (self.id,self.name, self.courseid)
        return '{ "id": "{}","name": "{}", "courseid" : "{}", "instructorid": "{}", "created_time": "{:%Y-%m-%d %H:%M}", "notebook_url": "{}"}'.format(self.id, self.name,self.courseid, self.instructorid, self.created_time, self.notebook_url)
